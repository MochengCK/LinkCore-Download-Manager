...
LinkCore Download Manager（联芯下载管理器）是一款简洁易用的跨平台下载器，采用现代 Web 技术构建。开箱即用的直观界面、清晰的任务流程与少量必要的设置，让下载管理变得更简单；同时支持常见下载协议与在线视频下载能力，覆盖日常使用场景。
***
...
### 🎬 视频下载
- **解析在线视频链接**：支持将网页中的视频地址一键解析为可下载任务
- **默认清晰度与格式**：在“偏好设置 → 视频设置”中预先配置默认清晰度与封装格式，新建任务时自动应用
- **下载后自动封装/转换**：视频下载完成后自动完成音视频合并，并按所选封装格式（如 MP4/MKV/MOV 等）输出
- **多任务视频批量管理**：在任务列表中统一查看、暂停/恢复和删除视频下载任务，支持与其他类型任务一致的管理体验
