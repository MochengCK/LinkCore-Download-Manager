...
def _get_bangumi_info_v2(epid=None, ssid=None, cookie=None):
  if epid:
    u = f'https://api.bilibili.com/pgc/view/web/season?ep_id={urllib.parse.quote(str(epid))}'
  elif ssid:
    u = f'https://api.bilibili.com/pgc/view/web/season?season_id={urllib.parse.quote(str(ssid))}'
...
def _parse_bangumi(final_url, epid, ssid, qn, cookie):
  ...
  if not episodes:
    if not cookie or 'SESSDATA=' not in cookie:
      raise ValueError('no episodes - login required (add SESSDATA cookie in settings)')
    ...
    debug_info = {
      ...
      'b_data_keys': list(b_data.keys()) if isinstance(b_data, dict) else [],
      'b_data': b_data,
      'has_cookie': True if cookie else False,
      'has_sessdata': True if (cookie and ('SESSDATA=' in cookie)) else False
    }
    
    error_msg = f'no episodes (url: {final_url}, epid: {epid}, ssid: {ssid}, code: {bangumi_code}, message: {bangumi_message}, data_keys: {list(b_data.keys()) if isinstance(b_data, dict) else []})'
